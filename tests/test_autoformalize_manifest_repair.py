"""Autoformalize-owned manifest repair contracts, without Lean, models, or remote calls."""
...
from unity import autoformalize_contract as contract, autoformalize_state as state
from unity import autoformalize_contract, autoformalize_representation, autoformalize_state
...
def formal_fixture(candidate, chunks, *, requirements=None, contract=None):
    ...
    refs = [row["ref_id"] for row in candidate["source_refs"]]
    requirements = deepcopy(requirements) if requirements is not None else [{
        "id": "R1", "statement": "The Lean result faithfully proves the supplied source.",
...
def machine_snapshot(state, report=None):
    ...
    result = {"snapshot_id": f"snapshot-{state['revision']}", "passed": True,
              ...
              "policy_sha256": autoformalize_contract.policy_hash(), "spec_sha256": digest(formal["spec"]),
              "repairs_sha256": autoformalize_state.repair_digest(state), "environment": deepcopy(contract.get("environment", {})),
              "representation_reviews": autoformalize_representation.snapshot(state),
              "external_declarations": autoformalize_contract._external_evidence(contract),
...
def semantic_evidence(state):
    return {"snapshot_id": state["formalization"]["review_snapshot"]["snapshot_id"],
            "scope_rationale": "The review covers the supplied source and its arguments.",
...
class ManifestStateFixture(unittest.TestCase):
    ...
    def setUp(self):
        temporary = tempfile.TemporaryDirectory(prefix="autoformalize-manifest-state-")
        ...
        # Compatibility attribute for shared test helpers; this is supplied provenance,
        # never a submitted/generated solution or an informal approval.
        self.paper = self.source
    ...
    def align(self, task_id):
        review = autoformalize_representation.current_representation_review(self.current(), task_id)
        if review and review["status"] == "pending":
            reviewer = "AlignmentReviewer"
            claimed = autoformalize_representation.claim_representation_review(self.forum, task_id, reviewer)
            self.assertEqual(claimed["status"], "claimed")
            payload = autoformalize_representation.representation_review_input(self.current(), task_id)
            autoformalize_representation.submit_representation_review(self.forum, reviewer, task_id, {
                "input_sha256": payload["input_sha256"], "verdict": "aligned",
                "checked_anchor_ids": [row["id"] for row in payload["anchors"]],
                "rationale": "The exact encoding preserves the supplied statement.",
                "evidence": "The test fixture binds each declaration to its immutable source anchor.",
            })

    def report(self, report=None):
        return machine_snapshot(self.current(), report)

    def semantic(self):
        return semantic_evidence(self.current())
...
class ManifestRepairStateTests(ManifestStateFixture):
    def test_round_activity_distinguishes_no_dispatch_from_attempted_work(self):
        current = state.record_round_end(self.forum, blocked_launches={"Ada": "preserved work"},
            activity={"worker_launches": 0, "integrations": 0, "review_launches": 0, "source_repairs": 0})
        self.assertEqual(current["formalization"]["last_round"]["outcome"], "blocked")
        self.assertEqual(current["formalization"]["last_round"]["blocked_launches"],
                         {"Ada": "preserved work"})
        current = state.record_round_end(self.forum, blocked_launches={}, activity={"review_launches": 1})
        self.assertEqual(current["formalization"]["last_round"]["outcome"], "attempted")
        self.accept("alpha")
        self.accept("beta")
        current = state.record_round_end(self.forum, blocked_launches={}, activity={})
        self.assertEqual(current["formalization"]["last_round"]["outcome"], "complete")
        self.assertEqual(current["phase"], "formalizing")
        self.assertNotEqual(current["formalization"]["status"], "accepted")

    def test_fixture_uses_supplied_source_and_real_targeted_review_policy(self):
        current = self.current()
        self.assertEqual(current["input_source"], self.source)
        self.assertEqual(state.formal_source(current)["kind"], "supplied_sources")
        self.assertEqual(current["solution"]["status"], "not_required")
        self.assertEqual(current["solution_candidates"], {})
        self.assertEqual(current["formalization"]["contract"]["representation_review_policy"], 1)
        self.assertTrue(state.interface_available(current, "alpha"))
        self.assertEqual(autoformalize_representation.current_representation_review(
            current, "alpha")["status"], "aligned")

    def test_manifest_repair_cannot_rewrite_supplied_source_or_obligations(self):
        self.preflight()
        before = self.current()
        self.assert_atomic_rejection(lambda: state.refine_chunks(
            self.forum, "Ada", before["revision"], {
                "source_obligations": {"requirements": [], "anchors": [], "scope": {}},
                "reopen_representations": [{"task_id": "alpha", "reason": "Change source instead."}],
            }))
        state.refine_chunks(self.forum, "Ada", before["revision"], {
            "reopen_representations": [{"task_id": "alpha", "reason": "Correct only the encoding."}],
        })
        after = self.current()
        self.assertEqual(after["input_source"], before["input_source"])
        self.assertEqual(after["formalization"]["source_obligations"],
                         before["formalization"]["source_obligations"])
        self.assertEqual(after["source_repairs"], before["source_repairs"])
        self.assertEqual(after["solution"], before["solution"])

    def test_replan_cannot_rewrite_frozen_supplied_source_obligation(self):
        before = self.current()
        replanning = deepcopy(before)
        replanning["phase"] = "chunking"
        dag = deepcopy(self.dag)
        dag["requirements"][0]["statement"] = "An easier replacement theorem."
        value = deepcopy(before["formalization"]["contract"])
        value["requirements"] = deepcopy(dag["requirements"])
        value["sha256"] = state._contract_digest(value)
        with self.assertRaisesRegex(ValueError, "cannot rewrite original source obligations"):
            state.prepare_informal_plan(replanning, dag,
                main_sha=before["formalization"]["main_sha"], contract=value)
    ...
    def test_repair_context_stales_on_each_bound_identity(self):
        ...
        mutations = {
            ...
            "source": lambda current: current["input_source"].update(sha256="4" * 64),
        }
