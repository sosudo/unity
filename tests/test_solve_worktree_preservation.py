"""Solve-only task-boundary and explicit synchronization guards; real Git, no Lean/model calls."""
...
from unity import artifacts, solve_representation, solve_state, worktree
from unity.forum import solve_server
from tests.solve_port_fixtures import initialize_graph
...
class SolveWorktreePreservationTests(unittest.TestCase):
    def setUp(self):
        ...
        artifact = artifacts.store_text(self.root / ".unity/artifacts", paper_text, kind="solution")
        solve_state.initialize(self.forum, hashlib.sha256((self.root / ".unity/UNITY.md").read_bytes()).hexdigest(), self.base, reset=True)
        self.paper = solve_state.submit_solution_candidate(
            self.forum, "Author", artifact["artifact_id"], artifact["sha256"], "paper.tex",
        )["candidate"]
        solve_state.review_solution_candidate(self.forum, self.paper["candidate_id"], "Reviewer", "approve", "Correct.")
        solve_state.accept_solution_candidate(self.forum, self.paper["candidate_id"], "Unity")
        initialize_graph(self.forum, [
            {"id": task, "dependencies": [], "lean_decl": task,
             "lean_file": task.title() + ".lean", "source_components": [f"paper:{self.paper['candidate_id']}"]}
            for task in ("existence", "sharpness", "next")
        ], solution_candidate=self.paper["candidate_id"], solution_sha256=self.paper["sha256"], main_sha=self.base)
        solve_server.configure(self.forum, self.root, "formalizing")
    ...
    def claim(self, task="existence", owner="Ada", assistant=""):
        ...
        solve_state.claim_strategy(self.forum, strategy["strategy_id"], owner)
        if assistant:
            solve_state.assist_strategy(self.forum, strategy["strategy_id"], assistant)
    ...
    def rejected_prerequisite(self):
        with solve_state.transaction(self.forum) as state:
            ...
            contract["sha256"] = solve_state._contract_digest(contract)
            ...
            row = solve_representation.queue_representation_review(state, "existence")
            state["representation_reviews"][row["input_sha256"]]["status"] = "encoding_error"
        current = solve_state.reconcile_rejected_representations(self.forum)
        return solve_state.current_manifest_repairs(current, "existence")[0]
    ...
    def test_exact_repair_can_checkpoint_proof_prerequisite_reassignment(self):
        ...
        result = solve_server.prepare_formal_worktree(
    ...
    def test_failed_repair_checkpoint_reset_retains_claim(self):
        ...
        with patch.object(solve_server, "_reset_formal_assignment", return_value={"ok": False}):
            result = solve_server.prepare_formal_worktree(
                "Ada", "sharpness", "existence", repair_id=repair["repair_id"],
                repair_input_sha256=repair["input_sha256"],
            )
    ...
    def test_failed_underlying_repair_reset_finishes_claim_release_on_retry(self):
        ...
        with patch.object(worktree, "force_sync_from_main", return_value={"ok": False}):
            first = solve_server.prepare_formal_worktree("Ada", "sharpness", "existence", **kwargs)
        self.assertFalse(first["ok"])
        current = solve_state.load_state(self.forum)
        self.assertEqual(current["strategies"][strategy_id]["status"], "claimed")
        second = solve_server.prepare_formal_worktree("Ada", "sharpness", "existence", **kwargs)
        ...
        third = solve_server.prepare_formal_worktree("Ada", "existence", "existence", **kwargs)
        self.assertTrue(third["ok"], third)
        self.assertEqual(solve_state.load_state(self.forum)["task_yields"]["ada"]["sharpness"], yielded)
    ...
    def test_repair_reset_interruption_before_forum_commit_recovers_related_release(self):
        ...
        kwargs = {"repair_id": repair["repair_id"], "repair_input_sha256": repair["input_sha256"]}
        with patch.object(solve_server, "_finish_repair_handoff", side_effect=RuntimeError("interrupted")):
            with self.assertRaisesRegex(RuntimeError, "interrupted"):
                solve_server.prepare_formal_worktree("Ada", "sharpness", "existence", **kwargs)
        self.assertEqual(solve_state.load_state(self.forum)["strategies"][strategy_id]["status"], "claimed")
        with patch.object(worktree, "force_sync_from_main") as sync:
            result = solve_server.prepare_formal_worktree("Ada", "sharpness", "existence", **kwargs)
        self.assertTrue(result["ok"], result)
        sync.assert_not_called()  # The successful Git reset is not repeated.
        current = solve_state.load_state(self.forum)
        self.assertEqual(current["strategies"][strategy_id]["status"], "registered")
        self.assertEqual(solve_server.unresolved_formal_tasks(current, "Ada"), [])
        checkpoint = solve_server._saved_task_checkpoint("Ada", "sharpness", current)
        self.assertEqual(git(self.root, "show", checkpoint["ref"] + ":Sharpness.lean"),
    ...
    def pending_repair_handoff(self):
        ...
        kwargs = {"repair_id": repair["repair_id"], "repair_input_sha256": repair["input_sha256"]}
        with patch.object(worktree, "force_sync_from_main", return_value={"ok": False}):
            self.assertFalse(solve_server.prepare_formal_worktree(
    ...
    def test_pending_repair_handoff_cannot_release_new_task_revision(self):
        ...
        with solve_state.transaction(self.forum) as current:
    ...
    def test_pending_repair_handoff_cannot_abandon_new_unrelated_claim(self):
        ...
        solve_state.yield_task(self.forum, "Ada", "sharpness", "Wait for its prerequisite.", ["existence"])
        strategy_id = self.claim("next")
        self.assertEqual(solve_server.unresolved_formal_tasks(solve_state.load_state(self.forum), "Ada"), ["next"])
        with patch.object(worktree, "force_sync_from_main") as sync:
            result = solve_server.prepare_formal_worktree("Ada", "sharpness", "existence", **kwargs)
        self.assertFalse(result["ok"])
    ...
    def test_stale_repair_permission_is_checked_before_worktree_mutation(self):
        ...
        with patch.object(solve_server, "_checkpoint_task_worktree") as checkpoint, \
    ...
    def test_same_task_preserves_dirty_and_committed_work_on_old_main(self):
        ...
        result = solve_server.prepare_formal_worktree("Ada", "existence", "existence")
    ...
    def test_previous_pending_unregistered_task_cannot_be_discarded(self):
        ...
        result = solve_server.prepare_formal_worktree("Ada", "existence", "sharpness")
        self.assertEqual(result["reason"], "unresolved_work")
    ...
    def test_unresolved_assistance_blocks_switch_after_other_task_completed(self):
        ...
        result = solve_server.prepare_formal_worktree("Ada", "sharpness", "next")
        self.assertEqual(result["reason"], "unresolved_work")
        self.assertEqual(solve_server.unresolved_formal_tasks(solve_state.load_state(self.forum), "ada"), ["existence"])

    def test_obsolete_and_released_assistance_does_not_block(self):
        strategy = self.claim("existence", "Bert", "Ada")
        solve_state.release_strategy(self.forum, strategy, "Bert")
        self.assertEqual(solve_server.unresolved_formal_tasks(solve_state.load_state(self.forum), "Ada"), [])
        with solve_state.transaction(self.forum) as state:
    ...
    def test_completed_assignment_can_reset_obsolete_work_and_retains_runtime_cache(self):
        ...
        result = solve_server.prepare_formal_worktree("Ada", "sharpness", "next", expected_revision=1)
    ...
    def test_fresh_clean_tree_fast_forwards_to_accepted_main(self):
        main = self.accept_main_change()
        result = solve_server.prepare_formal_worktree("Ada", next_task="existence")
        self.assertTrue(result["ok"])
        self.assertEqual(git(self.tree, "rev-parse", "HEAD"), main)

    def test_unassigned_dirty_or_committed_progress_is_not_discarded(self):
        (self.tree / "Existence.lean").write_text("unfinished\n")
        result = solve_server.prepare_formal_worktree("Ada", next_task="sharpness")
    ...
    def test_pending_candidates_protect_exact_branch_ancestry_and_claims(self):
        ...
        submitted = solve_server.emit_formalization_candidate(strategy, "Ada", "existence", sha)
        ...
        for status in ("submitted", "merging"):
            ...
            with self.subTest(status=status):
                sync = solve_server.sync_from_main("Ada")
                prepared = solve_server.prepare_formal_worktree("Ada", "sharpness", "next")
    ...
    def test_other_authors_or_obsolete_candidates_do_not_block(self):
        ...
        current = solve_state.load_state(self.forum)
        self.assertFalse(solve_server.has_pending_formal_candidate(current, "Ada"))
        self.assertTrue(solve_server.has_pending_formal_candidate(current))
        self.assertTrue(solve_server.sync_from_main("Ada")["ok"])

    def test_candidate_with_mismatched_paper_binding_is_not_current_work(self):
        with solve_state.transaction(self.forum) as state:
            state["formal_candidates"] = {
        ...
        self.assertFalse(solve_server.has_pending_formal_candidate(solve_state.load_state(self.forum)))
        self.assertEqual(solve_server.unresolved_formal_tasks(solve_state.load_state(self.forum), "Ada"), [])
        with solve_state.transaction(self.forum) as state:
            state["formal_candidates"]["stale"].update(solution_candidate=self.paper["candidate_id"], solution_sha256="c" * 64)
        self.assertFalse(solve_server.has_pending_formal_candidate(solve_state.load_state(self.forum)))
    ...
    def test_dirty_explicit_sync_preserves_bytes_and_claim(self):
        ...
        self.accept_main_change()
        result = solve_server.sync_from_main("Ada", "need accepted lemma")
        self.assertEqual(result["reason"], "dirty_worktree")
        self.assertEqual((self.tree / "Existence.lean").read_text(), "valuable progress\n")
        self.assertEqual(solve_state.load_state(self.forum)["strategies"][strategy]["status"], "claimed")

    def test_explicit_sync_merges_main_and_keeps_unsubmitted_commit_and_claim(self):
        strategy = self.claim()
        local = self.local_commit()
        main = self.accept_main_change()
        result = solve_server.sync_from_main("Ada")
        self.assertTrue(result["ok"])
    ...
    def test_explicit_sync_conflict_remains_in_worktree(self):
        ...
        self.assertEqual(solve_server.sync_from_main("Ada")["reason"], "dirty_worktree")
    ...
    def test_nondestructive_sync_does_not_overwrite_ignored_local_files(self):
        ...
        self.assertFalse(solve_server.sync_from_main("Ada")["ok"])
        self.assertFalse(solve_server.prepare_formal_worktree("Ada", next_task="existence")["ok"])
    ...
    def test_phase_revision_and_unaccepted_main_block_task_reset(self):
        ...
        self.assertEqual(solve_server.prepare_formal_worktree(
            "Ada", "sharpness", "next", expected_revision=0,
        )["reason"], "phase_changed")
        with solve_state.transaction(self.forum) as state:
            state["phase"] = "chunking"
        self.assertEqual(solve_server.sync_from_main("Ada")["reason"], "phase_changed")
        self.assertEqual(solve_server.prepare_formal_worktree("Ada", "sharpness", "next")["reason"], "phase_changed")
        with solve_state.transaction(self.forum) as state:
            state["phase"] = "formalizing"
        self.accept_main_change()
        with solve_state.transaction(self.forum) as state:
            state["formalization"]["main_sha"] = self.base
        self.assertEqual(solve_server.prepare_formal_worktree("Ada", "sharpness", "next")["reason"], "main_changed")
    ...
    def test_old_base_candidate_can_be_submitted_after_unrelated_main_merge(self):
        ...
        sha = self.local_commit()
        main = self.accept_main_change()
        result = solve_server.emit_formalization_candidate(strategy, "Ada", "existence", sha)
    ...
    def test_compatibility_submission_holds_author_lock(self):
        strategy = self.claim()
        sha = self.local_commit()
        original = solve_server._submit_formal_commit

        def check_lock(*args, **kwargs):
            with (self.forum / "solve-finalize-Ada.lock").open("a+") as lock:
                with self.assertRaises(BlockingIOError):
                    fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
            return original(*args, **kwargs)

        with patch.object(solve_server, "_submit_formal_commit", side_effect=check_lock):
            self.assertEqual(solve_server.emit_formalization_candidate(
    ...
    def test_destructive_prepare_holds_state_merge_and_author_locks(self):
        ...
        def check_locks(*args, **kwargs):
            for name in ("solve-state.lock", "merge.lock", "solve-finalize-Ada.lock"):
        ...
        with patch.object(worktree, "force_sync_from_main", side_effect=check_locks):
            self.assertTrue(solve_server.prepare_formal_worktree("Ada", "sharpness", "next")["ok"])
