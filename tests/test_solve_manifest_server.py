...
from unity.forum import solve_server as server
from unity.solve_review import RepresentationRepairRequest
...
class ManifestServerTests(unittest.TestCase):
    def setUp(self):
        ...
        self.stack.enter_context(patch.dict(os.environ, {
            "UNITY_AGENT_NAME": "", "UNITY_SOLVE_TASK_ID": "",
            "UNITY_SOLVE_BRIEF_CHARS": "12000",
        }))
    ...
    def test_verdict_forwards_requests_separately_from_semantic_evidence(self):
        ...
        with patch.object(solve_state, "submit_critic_verdict", return_value={"status": "recorded"}) as submit, \
    ...
    def test_verdict_rejects_nonlist_repairs_before_forwarding(self):
        ...
        with patch.object(solve_state, "submit_critic_verdict") as submit:
            for invalid in ({}, (), ""):
    ...
    def test_finalization_returns_preflight_block_before_any_git_or_worktree_access(self):
        for status in ("blocked", "conflict", "retry"):
            ...
            with self.subTest(status=status), \
                    patch.object(solve_state, "preflight_formal_submission", return_value=result, create=True), \
    ...
    def test_finalization_passes_exact_preflight_context_to_commit_submission(self):
        ...
        with patch.object(solve_state, "preflight_formal_submission", side_effect=preflight, create=True), \
                patch.object(solve_state, "load_state", return_value=self.state), \
                patch.object(solve_state, "strategy_is_current", return_value=True), \
                patch.object(solve_state, "participates", return_value=True), \
    ...
    def test_direct_commit_blocks_before_identity_reads_except_for_pending_identity_probe(self):
        with patch.object(solve_state, "preflight_formal_submission", return_value={"status": "blocked"}, create=True), \
    ...
    def test_direct_commit_keeps_exact_pending_and_accepted_retries_idempotent(self):
        for outcome in ("submitted", "already_adopted"):
            ...
            with self.subTest(outcome=outcome), \
                    patch.object(solve_state, "preflight_formal_submission",
                    ...
                    patch.object(server.solve_files, "immutable_git_paths", return_value={}), \
                    patch.object(server.solve_contract, "policy_hash", return_value="policy"), \
                    patch.object(solve_state, "submit_formal_candidate", return_value=expected) as submit, \
    ...
    def test_direct_commit_forwards_preflight_context_for_atomic_stale_check(self):
        ...
        with patch.object(solve_state, "preflight_formal_submission", return_value={"status": "ok", "context": context}, create=True), \
                patch.object(solve_state, "load_state", return_value=self.state), \
                patch.object(server.worktree, "verify_candidate_commit", return_value="a" * 40), \
                patch.object(server.worktree, "main_commit", return_value="b" * 40), \
                patch.object(server, "_git", return_value=SimpleNamespace(stdout="", returncode=0)), \
                patch.object(server.solve_files, "immutable_git_paths", return_value={}), \
                patch.object(server.solve_contract, "policy_hash", return_value="policy"), \
                patch.object(solve_state, "submit_formal_candidate", return_value={"status": "retry"}) as submit:
            result = server._submit_formal_commit("strategy", "Ada", "main", "a" * 40)
    ...
    def test_metrics_use_persisted_repairs_not_bounded_event_history(self):
        ...
        with patch.object(solve_state, "load_state", return_value=state), \
                patch.object(solve_state, "current_manifest_repairs", return_value=[state["manifest_repairs"]["waiting"]]):
    ...
    def test_task_and_brief_show_only_current_related_repairs(self):
        ...
        with patch.object(solve_state, "load_state", return_value=state), \
                patch.object(solve_state, "current_manifest_repairs", return_value=rows, create=True), \
                patch.object(server, "_detail", side_effect=lambda value, _: value):
            detail = server.solve_task("main")
            brief = server._formal_brief("Ada", "main")
