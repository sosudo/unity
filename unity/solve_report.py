"""Controller-written, evidence-bound reports for the solve workflow.
...
from . import artifacts, solve_contract, solve_state
...
def completion_report(state: dict, *, accepted: bool = True) -> dict:
    ...
    if accepted:
        ...
        solve_state._validate_snapshot_binding(state, snapshot, require_passed=True)
        ...
        if (not verdict or verdict.get("verdict") != "approved"
                ...
                or verdict.get("snapshot_sha256") != solve_state._report_digest(snapshot)
                or verdict.get("requirements_sha256")
                != solve_state._report_digest(formal.get("requirements", []))):
        ...
        solve_state._validate_semantic_review(
    ...
    return deepcopy({
        "schema_version": 1, "pipeline": "solve", "run_id": state.get("run_id"),
        "status": "accepted" if accepted else "incomplete", "phase": state.get("phase"),
        "original_source": solve_state.formal_source(state),
        "solution_gate": state.get("solution"),
        ...
        "source_repairs": repairs, "source_repairs_sha256": solve_state.repair_digest(state),
        ...
        "qualification": (
            ...
            "accepted immutable paper; source repairs are proposals requiring independent paper review."
        ),
    })
...
def persist_report(paths, *, accepted: bool = True) -> dict:
    ...
    """
    with _merge_lock(paths), solve_state.transaction(paths.forum) as state:
        ...
        record = artifacts.store_text(
            paths.artifacts, payload, kind="solve_completion_report", producer="Unity",
