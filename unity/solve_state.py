...
def _read_unlocked(forum_dir: Path) -> dict:
    ...
    for key in ("refinements", "formalization_history"):
...
def formal_source(state: dict) -> dict:
    """Bind formalization to independently accepted paper bytes, never drafts."""
    solution = state.get("solution", {})
    candidate = state.get("solution_candidates", {}).get(solution.get("accepted_candidate"), {})
    if solution.get("status") != "accepted" or candidate.get("status") != "accepted":
        return {}
    return {
        ...
        "source_refs": [{"ref_id": f"paper:{candidate['candidate_id']}",
                         "kind": "complete_paper", "path": ".unity/source/PROOF.tex",
                         "artifact_id": candidate["artifact_id"], "sha256": candidate["sha256"]},
                        *[{"ref_id": part["result_id"], "kind": part.get("kind", "component"),
                           "artifact_id": part["artifact_id"], "sha256": part["sha256"]}
...
def source_refs(state: dict) -> set[str]:
    """The accepted paper and incorporated components formalization must cover."""
...
def candidate_failure_key(candidate: dict, observation: dict | None) -> str | None:
    ...
    return digest({
        ...
        "obsolete_files": sorted(candidate.get("obsolete_files", []), key=lambda row: row["path"]),
    })
...
def strategy_is_current(state: dict, strategy: dict) -> bool:
    if strategy.get("phase") == "solving":
        return strategy.get("phase_revision") == state["solution"].get("revision")
...
def report_source_issue(forum_dir: Path, author: str, anchor_ids: list[str],
    ...
    with transaction(forum_dir) as state:
        if state["phase"] not in {"chunking", "formalizing", "critic"}:
            raise ValueError("source issues require an active solve formalization")
...
def ready_source_issues(state: dict) -> list[dict]:
    from .solve_representation import source_diagnosis_current

    return [item for item in open_source_issues(state) if item.get("status") == "open"
            or (item.get("status") == "proposed"
                and (source_diagnosis_current(state, item["issue_id"]) or {}).get("verdict") != "source_defect")]
...
def claim_source_issue(forum_dir: Path, issue_id: str, author: str,
    ...
    with transaction(forum_dir) as state:
        ...
        if not any(row["issue_id"] == issue_id for row in ready_source_issues(state)):
...
def finish_source_repair_attempt(forum_dir: Path, issue_id: str, author: str,
    ...
    with transaction(forum_dir) as state:
        issue = state["source_issues"].get(issue_id)
        if not issue:
            # A concurrent corrected-paper submission can retire this exact
            # worker between its final existence check and this transaction.
            # Preserve the archived evidence; never resurrect the old issue.
            for previous in reversed(state.get("formalization_history", [])):
                archived = (previous.get("source_issues") or {}).get(issue_id)
                if archived and any(
                    row.get("attempt_id") == attempt_id
                    and author_key(row.get("author")) == author_key(author)
                    for row in archived.get("attempts", [])
                ):
                    return {**deepcopy(archived), "status": "superseded", "owner": None}
            raise ValueError("unknown source issue")
        ...
        if attempt["status"] != "active":
            return deepcopy(issue)
        attempt.update(status="resolved" if issue.get("status") == "resolved" else
                       "proposed" if _live_repair_ids(issue) else "failed",
                       error=_text(error, "error", required=False), finished_at=time.time())
        if issue.get("status") not in {"resolved", "unresolved"} and author_key(issue.get("owner")) == author_key(author):
...
def mark_source_issue_unresolved(forum_dir: Path, issue_id: str, reason: str) -> dict:
    with transaction(forum_dir) as state:
        ...
        if not issue:
            raise ValueError("unknown source issue")
        if issue in ready_source_issues(state):
...
def submit_source_repair(forum_dir: Path, author: str, issue_id: str,
    ...
    with transaction(forum_dir) as state:
        ...
        # Evidence is not authority to amend accepted mathematics. A corrected
        # paper must enter propose_source_fix and the independent solution gate.
        repair["status"] = "proposal_only"
    ...
    *,
    ...
    with transaction(forum_dir) as state:
        ...
        strategy = {
            ...
            "task_revision": state["formal_tasks"].get(target, {}).get("revision") if phase == "formalizing" else None,
            "solution_candidate": formal_source(state).get("candidate_id") if phase == "formalizing" else None,
            "solution_sha256": formal_source(state).get("sha256") if phase == "formalizing" else None,
            "target": target,
            "description": description,
...
def assist_strategy(forum_dir: Path, strategy_id: str, author: str, contribution: str = "") -> dict:
    with transaction(forum_dir) as state:
        ...
        if (not strategy or strategy.get("status") != "claimed"
                or (strategy.get("phase") == "formalizing" and
                    (state["phase"] != "formalizing" or not strategy_is_current(state, strategy)))):
...
def _reopen_solution_in_tx(state: dict, author: str, reason: str) -> None:
    ...
    live_records = (
        "formal_tasks", "task_yields", "worker_tasks", "worktree_checkpoints",
        "retired_tasks", "refinements", "representation_reviews", "file_reservations",
        "source_issues", "source_repairs", "replan_requests", "replan",
    )
    state.setdefault("formalization_history", []).append({
        "solution_candidate": accepted,
        "reason": reason,
        "author": author,
        "timestamp": time.time(),
        "formalization": deepcopy(state["formalization"]),
        **{key: deepcopy(state.get(key)) for key in live_records},
    })
    _invalidate_review(state)
    for candidate in state["formal_candidates"].values():
        if candidate.get("status") in {"submitted", "merging"}:
            candidate.update(status="superseded", supersession_reason="accepted paper reopened")
    for attempt in state["chunking_attempts"]:
        if attempt.get("candidate_id") == accepted:
            attempt["obsolete"] = True
            if attempt.get("status") == "active":
                attempt.update(status="superseded", reason="accepted paper reopened", completed_at=time.time())
    defaults = _default_state()
    for key in live_records:
        state[key] = defaults[key]
    state.pop("chunking_failure", None)
    state["solution"] = {
    ...
    # The new paper has a fresh live DAG. Historical accepted candidates,
    # Lean commits, artifacts and archived graph evidence remain retrievable.
    for task in state["informal_tasks"].values():
...
def _replan_input(state: dict) -> str:
    """Meaningful plan inputs, excluding receipts, counters and event chatter."""
    formal = state["formalization"]
    contract = formal.get("contract") or {}
    return digest({
        "source": formal_source(state).get("sha256"),
        "spec": formal.get("spec"), "requirements": formal.get("requirements"),
        "tasks": {key: informal_interpretation_hash(task)
                  for key, task in state["formal_tasks"].items()},
        "bindings": contract.get("bindings"),
        "targets": {key: row.get("fingerprint") for key, row in contract.get("targets", {}).items()},
        "repairs": repair_digest(state),
    })
...
def _queue_replan(state: dict, author: str, reason: str, task_ids=None) -> dict:
    ...
    if existing:
        ...
        if reason != existing["reason"] and reason not in existing.setdefault("additional_reasons", []):
            existing["additional_reasons"].append(reason)
        return existing
    input_sha256 = _replan_input(state)
    for previous in state["replan_requests"].values():
        if (previous.get("status") == "complete" and previous.get("no_op")
                and previous.get("input_sha256") == input_sha256
                and set(previous["task_ids"]) == set(targets)
                and " ".join(previous["reason"].split()) == " ".join(str(reason).split())):
            return {**previous, "idempotent": True, "unchanged_plan": True}
    request = {"request_id": _id("replan"), "author": _text(author, "author", 100),
               "reason": _text(reason, "reason"), "task_ids": targets,
               "status": "queued", "created_at": time.time(), "input_sha256": input_sha256}
...
def _invalidate_formal_tasks(state: dict, seeds: set[str], *, reason: str) -> set[str]:
    """Invalidate representations, including actual kernel meaning dependencies."""
    from .solve_contract import invalidate_bindings
    ...
    for strategy in state["strategies"].values():
        if (strategy.get("phase") == "formalizing" and strategy.get("target") in affected
                and strategy.get("status") in _ACTIVE_STRATEGIES):
            strategy.update(status="cancelled", cancellation_reason=reason)
    _invalidate_review(state)
    return affected
...
def _adopt_repairs(state, spec) -> None:
    ...
    if adopted:
        raise ValueError("source repairs cannot amend accepted mathematics; use propose_source_fix "
                         "and obtain independent paper approval, or reopen_solving")
    if open_source_issues(state):
        raise ValueError("resolve source diagnosis before replanning; source defects require "
                         "a corrected, independently accepted paper")
...
def prepare_informal_plan(state: dict, dag: dict, *, main_sha: str, contract: dict,
    ...
    if (previous.get("contract") or {}).get("version") == 3:
        ...
        # Existing protected fingerprints must retain their serialization policy.
        contract["fingerprint_version"] = previous["contract"].get("fingerprint_version", 1)
        if "representation_review_policy" in previous["contract"]:
            contract["representation_review_policy"] = previous["contract"]["representation_review_policy"]
        else:
    ...
    if affected:
        affected = _invalidate_formal_tasks(state, affected, reason="informal interpretation changed")
    carried = [key for key, task in tasks.items() if task["status"] == "complete"]
    state["phase"] = "formalizing"
    from .solve_representation import queue_representation_review
    for key in tasks:
        queue_representation_review(state, key)
    replan = state.get("replan") or {}
    if replan:
        replan.update(status="complete", affected_tasks=sorted(affected), carried_tasks=carried)
        request = state["replan_requests"][replan["request_id"]]
        request.update(status="complete", no_op=request.get("input_sha256") == _replan_input(state))
...
def refine_chunks(forum_dir: Path, author: str, expected_revision: int, changes: ChunkRefinement) -> dict:
    ...
    with transaction(forum_dir) as state:
        ...
        if any(row["repair_ids"] for row in spec["arguments"]):
            raise ValueError("source repairs cannot amend accepted mathematics; use propose_source_fix")
        ...
        affected = _invalidate_formal_tasks(state, affected, reason="informal graph refined") if affected else set()
        for key in removed:
...
def initialize_formal_tasks(
    ...
    *,
    ...
    with transaction(forum_dir) as state:
        ...
        for strategy in state["strategies"].values():
            if (strategy.get("phase") == "formalizing"
                    and (strategy.get("target") in affected or not strategy.get("target"))
                    and strategy.get("status") in _ACTIVE_STRATEGIES):
...
def submission_blockers(state: dict, task_id: str, stage: str = "complete") -> list[dict]:
    ...
    if task_id not in tasks or stage not in {"representation", "complete"}:
        raise ValueError("submission preflight requires a known task and candidate stage")
...
def interface_available(state: dict, task: dict | str) -> bool:
    ...
    return (current_representation_review(state, task["task_id"]) or {}).get("status") == "aligned"
...
def task_ready(state: dict, task: dict | str) -> bool:
    ...
    if "statement_dependencies" in task:
        dependencies = set(task["statement_dependencies"])
        if (task.get("representation", {}).get("status") == "adopted"
                and (state.get("formalization", {}).get("contract") or {}).get("representation_review_policy") == 1):
            dependencies.update(task.get("proof_dependencies", []))
...
def _attempt_progress(state: dict, author: str, task_id: str) -> str:
    from .solve_files import reservations
    from .solve_representation import snapshot as representation_snapshot
    tasks = state.get("formal_tasks", {})
    ...
    return digest({
        ...
        "representation_reviews": {key: row for key, row in representation_snapshot(state).items()
                                   if key in relevant},
        "tasks": {key: {
...
def _task_strategies(state: dict, task_id: str) -> list[dict]:
    return [row for row in state.get("strategies", {}).values()
            if row.get("phase") == "formalizing" and row.get("target") == task_id
            and strategy_is_current(state, row)]
...
def assignment_view(state: dict, task_id: str) -> dict:
    strategies = [row for row in state.get("strategies", {}).values()
                  if row.get("phase") == "formalizing" and row.get("target") == task_id
                  and row.get("status") in {"claimed", "paused"}
...
def submit_formal_candidate(
    ...
    *,
    ...
    cleanup = solve_files.normalize_cleanup(obsolete_files)
    paths = solve_files.normalize_paths(changed_paths or [])
    deletions = solve_files.normalize_paths(deleted_paths or [])
    ...
    with transaction(forum_dir) as state:
        ...
        for existing in state["formal_candidates"].values():
            if (
                ...
                if (
                    ...
                    and existing.get("obsolete_files", []) == cleanup
                ):
        ...
        if inventory_observation is not None:
            if inventory_observation.get("state_context") != failure_state_context(state):
                return {"status": "retry", "task_id": task_id,
                        "next_action": "Accepted state changed during submission; refresh and retry."}
            blockers = solve_files.inventory_blockers(bindings, inventory_observation["project_declarations"])
            if blockers:
        ...
        proposal = {"task_id": task_id, "task_revision": task["revision"],
                    ...
                    "stage": stage, "outputs": bindings, "obsolete_files": cleanup}
        ...
        if stage == "representation" and representation_observation is not None:
            ...
            if (representation.get("status") == "adopted"
                ...
                return {
                    ...
                    "next_action": (
                        "This representation is adopted but its targeted review is not aligned yet. "
                        "Refresh solve_task; do not resubmit unchanged statements or begin dependent work."
                        if not interface_available(state, task) else
    ...
    *,
    ...
    with transaction(forum_dir) as state:
        ...
        if success:
            ...
            from .solve_contract import policy_hash
...
def _validate_snapshot_binding(state: dict, report: dict, *, require_passed: bool) -> None:
    ...
    if report.get("representation_reviews", {}) != representation_snapshot(state):
        raise ValueError("review snapshot has stale representation evidence")
