"""Task-local encoding review within the existing solve worker runtime.
...
from .solve_review import RepresentationReview, SourceDiagnosis
...
def _policy_hash() -> str:
    ...
    files = (Path(__file__), root / "solve_review.py",
             root / "prompts/solve/REPRESENTATION_REVIEW.md",
             root / "prompts/SOLVE_REPRESENTATION_REVIEW_TOOLS.md",
             root / "prompts/solve/SOURCE_REPAIR.md")
...
def submit_representation_review(forum_dir: Path, author: str, task_id: str, review: dict) -> dict:
    ...
    with module.transaction(forum_dir) as state:
        ...
        if evidence["verdict"] == "encoding_error":
            module._invalidate_formal_tasks(state, {task_id}, reason="Encoding review: " + evidence["rationale"])
    ...
    with module.transaction(forum_dir) as state:
        ...
        if evidence["verdict"] in {"false_alarm", "encoding_error"}:
            if evidence["verdict"] == "encoding_error":
                ...
                if targets:
                    module._invalidate_formal_tasks(state, targets, reason="Source diagnosis: " + evidence["evidence"])
            ...
            if evidence["verdict"] == "false_alarm":
                for task_id in state.get("formal_tasks", {}):
                    ...
                    if record and record.get("issue_id") == issue_id:
                        queue_representation_review(state, task_id)
        # A paper defect is not permission to change the formalization's source.
        # The repair worker must propose corrected paper bytes through the
        # independent solution gate, or explicitly reopen informal solving.
        module._invalidate_review(state)
...
async def representation_review_turn(agent, roster, paths, task_id: str,
    ...
    from .solve_formal_orchestrator import _preamble, build_solve_formal_mcp, stop_requested
    from .solve_formal_runtime import _agent_runtime_env, _formal_worktree
    ...
    try:
        ...
        artifact = artifacts.store_text(paths.project_root / ".unity" / "artifacts",
            json.dumps(material, sort_keys=True), kind="solve_representation_review",
            producer="Unity", source=record["input_sha256"])
        prompt_dir = Path(__file__).parent / "prompts"
        system = (_preamble(agent, roster, icrl_enabled=False)
                  + "\n" + (prompt_dir / "solve/REPRESENTATION_REVIEW.md").read_text()
                  + "\n" + (prompt_dir / "SOLVE_REPRESENTATION_REVIEW_TOOLS.md").read_text())
        env = _agent_runtime_env(paths, state, agent.name, task_id=task_id)
        env["UNITY_SOLVE_PROFILE"] = "representation_review"
        result = await spawn(agent, system,
            ...
            tree, build_solve_formal_mcp(paths, "representation_review"),
            interrupt_event=interrupt_event, env_overrides=env, own_process_group=True,
            mcp_profile="solve", log_context={"command": "solve", "run_id": state.get("run_id"),
