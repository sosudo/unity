"""Guard Lake commands launched by ``unity autoformalize`` formalization workers.

Autoformalize worktrees intentionally share ``.lake/packages`` with the main checkout.
...
from . import autoformalize_jobs
...
def _rejection(args: Sequence[str]) -> str | None:
    for command in _CONTROLLER_ONLY:
        if command in args:
            return (
                f"`lake {command}` is controller-only in unity autoformalize because "
    ...
    if exe is not None and exe + 1 < len(args) and args[exe + 1] == "cache":
        return (
            "`lake exe cache` is controller-only in unity autoformalize because the "
...
def run(
    ...
    *,
    ...
    project_root = str(env.get("UNITY_AUTOFORMALIZE_PROJECT_ROOT") or "").strip()
    if not real_lake or not Path(real_lake).is_file():
        print("error: autoformalize Lake guard has no valid UNITY_REAL_LAKE", file=sys.stderr)
        return 2
    if not project_root or not Path(project_root).is_dir():
        print("error: autoformalize Lake guard has no valid UNITY_AUTOFORMALIZE_PROJECT_ROOT", file=sys.stderr)
        return 2
    ...
    completed = autoformalize_jobs.run(
        Path(project_root),
        [real_lake, *args],
        cwd=Path(cwd or Path.cwd()),
        owner=str(env.get("UNITY_AGENT_NAME") or "autoformalize-worker"),
        task_id=str(env.get("UNITY_AUTOFORMALIZE_TASK_ID") or ""),
