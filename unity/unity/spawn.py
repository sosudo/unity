...
async def _aiter_sync(sync_iterable):
    """Bridge a blocking iterator into an async one, one item per thread hop."""
...
async def codex_spawner(agent: Agent, system_prompt: str, prompt: str, cwd: Path,
    ...
    sandbox = openai_codex.Sandbox.read_only if permission == "acceptEdits" \
        else openai_codex.Sandbox.workspace_write

    codex = openai_codex.Codex(
        config=openai_codex.CodexConfig(cwd=str(cwd), env=_agent_env(agent, home))
    )
    if agent.api_key:
