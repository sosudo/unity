...
def policy_hash() -> str:
    """Bind reuse to the current solve checking implementation."""
    directory = Path(__file__).parent
    policy = {name: hashlib.sha256((directory / name).read_bytes()).hexdigest() for name in (
        "solve_contract.py", "solve_contract.lean",
        "solve_spec.py", "solve_formal_runtime.py", "solve_state.py",
        "solve_workspace.py", "solve_workspace.lean",
        "solve_native.py", "solve_jobs.py", "solve_cache.py",
    )}
...
def inspect_environment(root: Path, tasks: list[dict], *, layout: dict | None = None,
    ...
    if (root / ".git").exists():
        try:
            ...
            cached, cache_before = solve_cache.lookup(root, cache_key)
    ...
    with measure(timings, "inspector_seconds"):
        result = (subprocess.CompletedProcess([], 0, json.dumps(cached), "") if cached is not None else solve_jobs.run(
    ...
    def failure(message: str, declaration_errors: list[dict] | None = None) -> ValueError:
        record = artifacts.store_text(
            ...
            kind="solve_inspection", producer="Unity", source="formal contract inspector",
        )
    ...
    if cache_key is not None and cached is None:
        try:
            if source_identity(root) == cache_identity:
                compiled = solve_cache.publish(root, cache_key, data, cache_before)
        except solve_cache.CacheUnavailable as exc:
            ...
            pass
    data["compiled_receipt"] = solve_cache.compiled_receipt(root, compiled)
    return data
...
def freeze_formal_contract(paths, dag: dict) -> dict:
    ...
    source = solve_state.formal_source(solve_state.load_state(paths.forum))
...
def prepare_source_contract(paths, dag: dict, *, state: dict | None = None,
    ...
    state = solve_state.load_state(paths.forum) if state is None else state
...
def initialize_source_contract(paths, dag: dict) -> dict:
    ...
    record = artifacts.store_text(paths.artifacts, json.dumps({"contract": contract}, sort_keys=True) + "\n",
                                  kind="solve_source_contract", producer="Unity")
...
def _check_incremental_contract(root: Path, contract: dict, tasks: list[dict], *, completed: set[str],
    ...
    try:
        ...
        witness_records = {name: {**row, "fingerprint": digest(_semantic_record(row, fingerprint_version=fingerprint_version))}
                           for name, row in witness_records.items()}
    except solve_jobs.JobCancelled:
        raise
...
def snapshot_is_current(paths, state: dict, snapshot: dict, *, require_complete: bool = True) -> bool:
    from .solve_input import source_matches
    from .solve_representation import snapshot as representation_snapshot
    from .solve_state import formal_source, repair_digest
    ...
    return (
        ...
        and (snapshot.get("passed") is not True
             or solve_cache.compiled_receipt_current(paths.project_root, snapshot.get("compiled_receipt")))
...
def verify_final_project(paths, state: dict) -> dict:
    from .solve_input import source_matches
    from .solve_representation import snapshot as representation_snapshot
    from .solve_state import interface_available, repair_digest
    ...
    reusable = (build_reusable
                and solve_cache.compiled_receipt_current(root, verification.get("compiled_receipt"))
