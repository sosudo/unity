# Available tools for `unity solve` — optional source repair

Use `solve_brief(author)` for the current issue/repair state and `solve_task(task_id)`
...
- `submit_source_repair(author, issue_id, explanation, evidence, replacement?)` records an immutable,
  source-bound proposal. It is diagnostic evidence, not authority to amend the accepted mathematics
  or adopt a changed paper through chunking. State exactly what changes, if anything, and cite evidence.
- `propose_source_fix(author, path, reason, supersedes?)` submits a corrected full paper draft for
  independent solution review. Preserve accepted `PROOF.tex`; do not approve your own correction.
- `reopen_solving(author, reason)` returns substantial missing mathematics to informal solving.
  End this role when the phase changes. Keep source argument `repair_ids` empty.
...
No formal candidate finalization or accepted-source rewriting belongs to this role. A separate corrected
paper draft may be prepared for independent review. Keep detailed search output in artifacts, and leave
existing worktree proof files unchanged.
