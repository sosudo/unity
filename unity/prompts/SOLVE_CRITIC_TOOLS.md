...
Start with `solve_brief(author)` and use `solve_status()` for exact task/candidate state. Inspect the
accepted-paper snapshot, original problem and current review artifacts using `artifact_info`, bounded `artifact_read`,
...
- `submit_formalization_verdict(author, verdict, summary, review, reopen_tasks?, evidence?)` submits
  ...
  a global `scope_rationale`, and empty `repair_reviews` (changed paper mathematics requires independent
  solution review, not repair-ID adoption). Approval requires every recorded requirement
...
- `solve_requirements(offset?, limit?)` pages the complete requirement manifest. Keep one revision
  and continue until `next_offset` is null; the task-filtered brief is not the entire checklist.
  `solve_task(task_id)` retrieves exact task evidence/anchors/prerequisites.
...
- `propose_source_fix(author, path, reason, supersedes?)` submits already-prepared corrected full
  paper bytes for independent solution review. The critic must not write or approve its own correction.
- `reopen_solving(author, reason)` returns substantial gaps in the accepted mathematics to informal
  solving. End this review when the phase changes; do not issue approval for the old snapshot.
...
Paper corrections return through the existing informal solution gate, never through a weakened Lean
encoding or unapproved repair proposal. Report substantive source defects
