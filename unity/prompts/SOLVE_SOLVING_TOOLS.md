...
- `create_informal_task(author, kind, title, description, dependencies?, parent_task?)` records a lemma,
  ...
  concise agent-chosen identifiers. `create_subgoal` remains a shorthand for a subgoal task.
...
- `publish_finding(author, kind, title, content, confidence, target?, strategy_id?, evidence?)` publishes
  live knowledge. Kinds are agent-chosen; confidence is an integer from 0 to 100.
