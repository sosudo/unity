...
`autoformalize_task(task_id)` provides current context, but submit only for the assigned input hash.
`autoformalize_requirements` retrieves original source anchors and requirements; `read_finding`
