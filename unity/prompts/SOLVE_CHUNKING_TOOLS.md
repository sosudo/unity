...
The Forum tools in this profile are read-only. Start with `solve_brief(author)` and inspect the exact accepted solution with
...
`.unity/dag.json`, including its `requirements`, and create the Lean statement scaffold in project files.
Unity builds and snapshots the scaffold before formalization; no Forum mutation is required.
