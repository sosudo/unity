...
Start with `solve_brief(author)` and the absolute formalization-plan path in your task for the immutable accepted-paper
snapshot, file paths/artifacts and exact source-reference IDs. Read source files directly as appropriate
to their format; use `artifact_info` and bounded `artifact_read` for stored text. `solve_status()` exposes
...
the independently accepted solution candidate and exact paper SHA-256.
...
Inspect existing proposals as diagnostic evidence, not amendments to the accepted paper. Keep
`spec.arguments[].repair_ids` (or `arguments[].repair_ids` on replan) empty.
`propose_source_fix(author, path, reason, supersedes?)` submits a corrected full paper draft for
independent solution review; `reopen_solving(author, reason)` returns substantive gaps to informal work.
Neither a repair proposal nor a changed DAG can approve changed mathematics.
