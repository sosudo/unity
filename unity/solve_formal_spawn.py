"""Solve formalization-owned agent launcher, copied from the working formalization backend.
...
helpers are independent copies, not the informal solve/prove launch implementation.
"""
...
def _worktree_write_roots(cwd: Path) -> tuple[Path, ...]:
    ...
    if any(path != cwd and cwd.is_relative_to(path) for path in roots):
        raise ValueError("solve shared write access cannot include the source checkout")
...
def _process_group_wrapper(executable: Path, directory: Path, label: str) -> tuple[Path, Path]:
    """Wrap a CLI so its entire process tree has an solve-owned process group."""
...
async def claude_spawner(agent: Agent, system_prompt: str, prompt: str, cwd: Path,
    ...
    if roots:
        from .solve_permissions import claude_worktree_options
...
_SOLVE_CODEX_MCP_NOTE = (
    ...
    "registering an solve strategy. Only call tools exposed for your current phase.\n"
)
...
def _codex_mcp_note(profile: str, phase: str | None = None) -> str:
    if phase == "chunking":
        return (
            ...
            "For example: unity mcp unity-forum solve_brief "
...
async def codex_spawner(agent: Agent, system_prompt: str, prompt: str, cwd: Path,
                        ...
                        env_overrides: dict[str, str] | None = None,
                        own_process_group: bool = False,
                        mcp_profile: str = "solve",
                        on_normal_completion: CompletionCallback | None = None) -> str | None:
    from openai_codex import AsyncCodex, CodexConfig, Sandbox

    system_prompt = system_prompt + _codex_mcp_note(
        mcp_profile, (env_overrides or {}).get("UNITY_SOLVE_PROFILE"),
    )
...
async def spawn(agent: Agent, system_prompt: str, prompt: str, cwd: Path,
    ...
    try:
        ...
        if phase and "UNITY_SOLVE_PROFILE" not in (env_overrides or {}):
            env_overrides = {**(env_overrides or {}), "UNITY_SOLVE_PROFILE": phase}
        # Other solve phases keep their own prompts, without proof-development catalogs.
        if (mcp_profile == "solve"
                and (env_overrides or {}).get("UNITY_SOLVE_PROFILE", phase) == "formalizing"):
