"""Source-evidence bookkeeping for solve formalization, not an English proof checker.
...
def normalize_spec(value, *, source, requirements, tasks, allow_unresolved: bool = False) -> dict:
    ...
    if type(value["version"]) is not int or value["version"] != 1:
        raise ValueError("unsupported solve spec version; re-chunk with source evidence")
    tasks = _task_map(tasks)
