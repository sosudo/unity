"""Event-driven formalization workers for ``unity solve``.
...
from . import solve_files, solve_representation
from .solve_input import require_source_matches
from .forum import solve_server
from .solve_formal_orchestrator import _preamble, load_prompt, stop_requested
...
def forum_brief(paths, profile: str, author: str, task_id: str = "") -> str:
    ...
    try:
        return solve_server.solve_brief(author, task_id=task_id)
    except Exception:
        return ""
...
def normalize_chunking_dag(dag: dict, *, plan: dict, expected_solution_sha: str) -> dict:
    ...
    from .solve_spec import normalize_requirements, normalize_spec, normalize_informal_nodes
...
def assemble_chunking_draft(state: dict, draft: dict) -> dict:
    from .solve_spec import PlanValidationError, _object
...
def read_chunking_draft(path: Path) -> bytes:
    ...
    from .solve_spec import PlanValidationError
...
def prepare_chunking_draft(paths, state: dict, payload: bytes) -> tuple[dict, dict]:
    ...
    contract = solve_contract.prepare_source_contract(
        paths, dag, state=state, environment=environment, main_sha=main_sha)
    solve_state.prepare_informal_plan(state, dag, main_sha=main_sha, contract=contract)
    return dag, contract
...
def _rollback(root: Path, before: str) -> subprocess.CompletedProcess:
    with solve_jobs.cancellation_disabled():
...
def _formal_worktree(project_root: Path, author: str) -> Path:
    ...
    if not tree.exists() and not tree.is_symlink():
        if _git(project_root, "show-ref", "--verify", "--quiet", f"refs/heads/{branch}").returncode == 0:
            raise ValueError(f"Preserved branch {branch} has no worktree; reconcile it before resuming solve")
    ...
    if (tree.is_symlink() or registered.returncode or not matching or top is None or top.returncode
        ...
        raise ValueError(f"Existing path {tree} is not the expected registered worktree for {author}; "
                         "preserved unchanged. Reconcile it before resuming solve")
    return tree
...
def _apply_formal_candidate(paths, candidate: dict, task: dict, *, timings: dict | None = None) -> dict:
    ...
    if not solve_state.candidate_is_current(current, candidate):
    ...
    if blockers:
        ...
        return {"ok": False, "error": "; ".join(issues), "blockers": blockers,
                "failure_context": solve_state.failure_state_context(current),
    ...
    actual_paths = solve_files.immutable_git_paths(root, candidate["base_main_sha"], resolved)
    blockers = solve_files.validate_candidate_files(current, candidate, **actual_paths)
    if blockers:
    ...
    verification["environment_sha256"] = solve_contract.digest(checked_source["environment"])
    ...
    try:
        ...
        if any(row.get("status") == "failed" and row.get("task_id") == candidate["task_id"]
            ...
            try:
                ...
                previous = solve_state.matching_failed_candidate(current, candidate, observation)
        ...
        if (not result.get("ok") and blockers
            ...
            try:
                observation = solve_contract.observe_failure_inputs(
                    root, solve_state.load_state(paths.forum)["formalization"].get("contract") or {},
                )
        ...
        result = {"ok": False, "error": f"candidate verification failed: {exc}",
                  "cancelled": isinstance(exc, solve_jobs.JobCancelled)}
    finally:
...
def _integrate_and_record(paths, candidate: dict, task: dict, cancel_event: Event | None = None) -> dict:
    ...
    def record(result: dict) -> dict:
        if result.get("cancelled") and solve_state.pending_replan(solve_state.load_state(paths.forum)):
            solve_state.defer_formal_merge(paths.forum, candidate["candidate_id"], reason=result["error"])
    ...
    with solve_jobs.cancellation_scope(cancel_event):
        try:
            with _merge_lock(paths.project_root):
                try:
                    result = _integrate_checked(paths, candidate, task)
                except solve_jobs.JobCancelled as exc:
...
def _rejection_recovery_prompt(state: dict, author: str, task_id: str) -> str:
    ...
    relevant = [
        ...
        and (item.get("status") == "merged" or (
            ...
            and (solve_state.author_key(item.get("author")) == solve_state.author_key(author)
                 or solve_state.participates(
...
def _formal_launch_retry_key(state: dict, author: str, task_id: str, previous_task: str) -> str:
    ...
    return solve_state.digest({
        ...
        "tasks": {key: {
            ...
            "attempt": solve_state.snapshot_attempt(state, author, key),
        ...
        "previous_yielded": solve_state.has_yielded(state, author, previous),
        "unresolved": solve_server.unresolved_formal_tasks(state, author),
...
async def run_formalizing_runtime(roster, paths, mcp: dict, base_prompt: str) -> dict:
    ...
    def participating_strategy(current: dict, name: str, task_id: str = "") -> dict | None:
        matches = [
            ...
            and solve_state.strategy_is_current(current, strategy)
    ...
    def launch(name: str, task_id: str, followup: str = "") -> None:
        ...
        repair_blockers = [row for row in solve_server.verification_blockers(current, task_id)
        ...
        if not prepared["ok"]:
            ...
            blocked_launch_keys[pair] = _formal_launch_retry_key(
                solve_state.load_state(paths.forum), name, task_id, worker_targets.get(name, ""),
            )
        ...
        worker_attempts[name] = solve_state.snapshot_attempt(current, name, task_id)
        ...
        if brief:
            system += f"\n{PIPELINE.capitalize()} workspace brief (refresh with solve_brief):\n{brief}\n"
        system += base_prompt + "\n\n" + tools_prompt
        if context:
            system += "\n\n" + context
        ...
        normal_task_prompt = (
            ...
            "as you work. The accepted PROOF.tex and original UNITY.md are read-only. "
            "Use report_source_issue for suspected paper defects. A corrected paper must go through "
    ...
    def launch_idle() -> None:
        ...
        for review in solve_representation.pending_representation_reviews(current):
            if review["input_sha256"] in {key for _, key in review_inputs.values()}:
                continue
            tried = solve_representation.attempted_reviewers(review)
            available = [name for name in agents if name not in tasks and name not in stopping
                         and solve_state.author_key(name) not in tried
                         and not solve_server.has_pending_formal_candidate(current, name)]
            available.sort(key=lambda name: solve_state.author_key(name)
                           == solve_state.author_key(review.get("representation_author")))
            if not available:
                continue
            name = available[0]
        ...
        for name in agents:
            if name in tasks or name in stopping or solve_server.has_pending_formal_candidate(current, name):
                continue
        ...
        for name in idle:
            ...
            if successor:
                launch(name, successor, "Your prior informal node was replaced. Read solve_task "
            ...
            if current["formal_tasks"].get(previous, {}).get("status") == "pending":
                if previous in ready_ids and solve_state.task_available_to(current, name, previous):
                    launch(name, previous)
                    continue
                if not solve_server.unresolved_formal_tasks(current, name):
                    prerequisites = solve_server.ready_statement_prerequisites(current, previous)
                    if solve_state.has_yielded(current, name, previous):
                    ...
                    for _, prerequisite in _formal_task_assignments(
                        ...
                        available_to=lambda owner, target: solve_state.task_available_to(current, owner, target),
                    ):
    ...
    try:
        ...
        while not stop_requested(paths.project_root):
            ...
            for name in list(tasks):
                if roles.get(name) == "representation_review":
                    ...
                    latest = solve_representation.representation_review_input(state, target)
            ...
            replan = solve_state.pending_replan(state)
            ...
            for name, running in list(tasks.items()):
                ...
                if role == "formalizing" and ended_normally and not interrupted and attempt:
                    ...
                    submitted = any(
                        ...
                        and solve_state.author_key(candidate.get("author")) == solve_state.author_key(name)
                        for key, candidate in current.get("formal_candidates", {}).items()
                    )
                    if not submitted and not solve_server.has_pending_formal_candidate(current, name):
                        solve_state.record_worker_yield(
            ...
            if integration is not None and integration.done():
                ...
                if result.get("ok"):
                    ...
                    if latest.get("phase") == "formalizing":
            ...
            if replan:
                if integration is None and not tasks and not stopping:
                    ...
                    with _merge_lock(paths.project_root):
                        return solve_state.begin_replan(
            ...
            if integration is None:
                ...
                for candidate in candidates:
                    ...
                    if any(active_target(name) == candidate["task_id"]
                        ...
                        continue
                    started = solve_state.begin_formal_merge(paths.forum, candidate["candidate_id"])
            ...
            state = solve_state.load_state(paths.forum)
            if (integration is None and not tasks and not stopping
                    and solve_state.all_formal_tasks_complete(state)):
        ...
        for agent in roster.agents:
            ...
            if running is not None and not running.done():
                ...
                continue
            if (not solve_state.all_formal_tasks_complete(final_state)
                    or final_state.get("phase") == "chunking"
                    or solve_state.pending_replan(final_state)
                    or solve_state.open_source_issues(final_state)):
                continue
    ...
    with _merge_lock(paths.project_root):
        for author, assignment in saved.items():
            if assignment.get("task_id") not in affected:
                continue
            with solve_server._finalization_lock(author):
                ...
                if status.stdout.strip():
                    ...
                    committed = _git(tree, "commit", "-m", "UNITY: preserve obsolete solve work before replanning")
                ...
                reference = f"refs/unity/solve-checkpoints/{safe_author}/{uuid.uuid4().hex}"
...
def refresh_replanned_worktrees(paths, assignments: dict, state: dict) -> dict:
    ...
    with _merge_lock(paths.project_root):
        for author, assignment in saved.items():
            ...
            with solve_server._finalization_lock(author):
                current = solve_state.load_state(paths.forum)
                if solve_server.has_pending_formal_candidate(current, author):
